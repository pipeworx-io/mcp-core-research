 ...
 *
 * BYO key: requires a free CORE API key from https://core.ac.uk/services/api
 * Passed via _apiKey parameter.
 *
 * Tools:
 * - search_papers: search open-access research papers by keyword
 * - get_paper: get full metadata for a paper by CORE ID
 */


/**
 * LICENCE (fleet #1974). CORE's Terms & Conditions (https://core.ac.uk/terms)
 * §3 "Other datasets and the API": the API needs a licence, free licences go
 * to individuals and public research organisations who "will acknowledge the
 * use of CORE", and anyone "developing or intending to develop a product,
 * service or software using CORE data which might be monetised now or in the
 * future" must contact CORE. This pack is BYO-key: the caller's own CORE
 * licence governs the call. We charge nothing for it (`zeroRated: true` on the
 * gateway entry) so the gateway never becomes the monetised product §3
 * describes, and every response carries the acknowledgement CORE asks for.
 */
const CORE_LICENSE = vendorTermsLicense({
  ref: 'CORE-Terms-and-Conditions',
  url: 'https://core.ac.uk/terms',
  attribution: 'Data from CORE (core.ac.uk), The Open University — aggregated open-access research; served under your own CORE API licence',
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'CORE Research');
}
...
function extractKey(args: Record<string, unknown>): string {
  ...
  delete args._apiKey;
  if (!key) throw new Error('CORE API key required. Get one free at https://core.ac.uk/services/api and pass via _apiKey.');
  return key;
}

async function coreGet(apiKey: string, path: string, params?: Record<string, string>): Promise<unknown> {
...
const tools: McpToolExport['tools'] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        _apiKey: { type: 'string', description: 'CORE API key' },
        query: {
          type: 'string',
          description: 'Search query (e.g., "climate change mitigation", "CRISPR gene editing")',
        },
        limit: {
          type: 'number',
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        id: {
          type: 'string',
          description: 'CORE paper ID (numeric string)',
        },
      },
      required: ['_apiKey', 'id'],
    },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  // Attached here, not per handler, so no tool or early return can ship
  // unattributed by omission (the mcps/finra shape, fleet #531).
  return attachLicense(await dispatch(name, args), CORE_LICENSE);
}
...
async function dispatch(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  switch (name) {
    case 'search_papers':
      return searchPapers(key, args.query as string, (args.limit as number) ?? 10);
    case 'get_paper':
      return getPaper(key, args.id as string);
    default:
...
async function searchPapers(apiKey: string, query: string, limit: number) {
  const safeLimit = Math.min(100, Math.max(1, limit));
  const data = (await coreGet(apiKey, '/search/works', {
    q: query,
...
async function getPaper(apiKey: string, id: string) {
  const p = (await coreGet(apiKey, `/works/${id}`)) as {
    id: string;
    title: string;
    authors: Array<{ name: string }>;
    abstract: string;
  ...
  return {
    ...
    references_count: (p.references ?? []).length,
  };
}
